...
Page({
  ...
  jzySwitch: function (e) {
    this.setData({
      jzy: e.detail.value
    })
    this.updatePorperty()
  },
  yanguangSwitch: function (e) {
    this.setData({
      yanguang: e.detail.value
    })
    this.updatePorperty()
  },
  chongmingSwitch: function (e) {
    this.setData({
      chongming: e.detail.value
    })
    this.updatePorperty()
  },
  //onload 
  onLoad: function () {
    ...
    db.collection('equipment').where({
      ...
      success: res => {
        var list = []
        var tempEquipList = res.data[0].equipListSHD
  ...
  drawCanvas: function () {
    ...
    Promise.all(
      ...
      ctx.setFontSize(24)
      ctx.fillText('门派：御剑    专精：神虹诀', 450, 80)
